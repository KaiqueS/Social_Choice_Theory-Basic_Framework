...
void initialize_graph( std::vector<Agent>& listofagents, std::vector<SocialPrefNode>& graph ){
    ...
    std::vector<int>::size_type index = static_cast<std::vector<int>::size_type>( rand( ) ) % listofagents.size( );
    ...
    for( std::vector<int>::size_type i = 0; i < ordering.size( ); ++i ){
        ...
        for( std::vector<int>::size_type j = 0; j < listofagents.size( ); ++j ){
            ...
            for( std::vector<int>::size_type k = 0; k < listofagents[ j ].get_preferences( ).size( ); ++ k ){
                ...
                else if( listofagents[ j ].get_preferences( )[ k ].get_opt( ) == compairs.ypref.get_opt( ) )
